...
class WalletService {
  ...
  async findWalletByIdentifier(alias: string): Promise<Wallet | null> {
    try {
      // Convert alias to lowercase to match database format
      const normalizedAlias = alias.toLowerCase().trim();
      const response = await apiRequest(`/wallets/alias/${normalizedAlias}`, {
        method: "GET",
      });
  ...
  async transferBetweenUsers(
    ...
    try {
      ...
      if (!recipientWallet) {
        ...
        throw new Error(
          `No se encontró ninguna billetera con el alias "${recipientIdentifier}". Verifica que el alias sea correcto.`
        );
      }
